...
/**
 * Delete the derived index so it gets rebuilt from the card.
 *
 * library.idx is a cache: every byte of it can be regenerated by walking the media folder. stats.bin
 * cannot - it holds favourites, play counts and resume positions - so it stays, which is exactly why
 * the two were split into separate files in the first place.
 */
void media_library_clear_cache(void)
{
    char temp_path[LIB_PATH_MAX + 8];

    media_library_scan_stop();
    media_library_commit(); // Flush stats before touching anything on disk
    index_close();
    clear_index();

    if (!lib.index_path[0])
        return; // Never initialised, nothing of ours is on the card

    snprintf(temp_path, sizeof(temp_path), "%s.tmp", lib.index_path);
    rg_storage_delete(lib.index_path);
    rg_storage_delete(temp_path);

    RG_LOGI("Media library index cleared");
}
